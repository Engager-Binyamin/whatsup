  ...
  if (client && client.isReady) {
    try {
      ...
      const ackListener = async (msg, ack) => {
        const campaign = await campaignController.readOne({
          _id: newData.campaignId,
        });
        if (!campaign) throw "error";
        let received = campaign.receivedMsgs.find(
          (re) => String(re._id) == String(newData.receivedId)
        );
        ...
        if (msg.id.id === messageId) {
          if (ack === 1) {
            ...
            console.log(
              `Message with ID ${messageId} was received by ${chatId}`
            );
